...
class MultitrackItem():
    ...
    @classmethod
    def from_audio(cls, audiofile, chordfile, vocab, stream=None):
        # convert to numpy
        chords = encode_chords_from_txt_file(chordfile)
        melody = encode_melody_from_audio(audiofile)
        return MultitrackItem(MusicItem.from_npenc(melody, vocab), MusicItem.from_npenc(chords, vocab), stream)        
